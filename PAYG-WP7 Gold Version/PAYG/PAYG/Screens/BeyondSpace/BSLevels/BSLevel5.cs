...
namespace PAYG
{
    public class BSLevel5 : GameScreen
    {
        #region Fields
        ...
        GameObject[] EnemyType1 = new GameObject[14];
        GameObject[] EnemyType2 = new GameObject[12];
        GameObject[] EnemyType3 = new GameObject[9];
        GameObject[] EnemyType4 = new GameObject[9];
        GameObject[] EnemyType5 = new GameObject[8];
        GameObject[] enemyBullets = new GameObject[10];
        ...
        int numMissles = 0;
        int missleBonus = 2000;
        int toMissleBonus = 2000;
        ...
        int span1 = 3;
        double toNextSpawn1 = 3;

        double span2 = 4.2;
        double toNextSpawn2 = 4.2;

        double span3 = 6.4;
        double toNextSpawn3 = 6.4;

        double span4 = 6.6;
        double toNextSpawn4 = 6.6;

        int span5 = 4;
        int num5Spawns;  //needed to ensure the swarm is accounted for
        int num5Count = 0;  //control variable for non-swarm enemies
        double toNextSpawn5 = 4.2;

        int spanSwarm = 30;
        double toNextSwarm = 30;
        int swarmCount = 0; //control variable for swarm enemies
        int swarmTimes = 0;
        ...
        double nonShootingTimer;
        double bulletInterval = 0.5d;
        bool shooting = false;
        bool notShooting = true;
        Vector2 lastVel = new Vector2(2, 0);
        int hitPoints;
        ...
        /// Constructor
        /// </summary>
        public BSLevel5(int sc, int life, bool blaster, int numMissles, int bBonus, int mBonus,
        ...
        public override void Initialize()
        {
            ...
            boss = new GameObject(newSheet, new Rectangle(625, 677, 455, 182), 1, true, 0);
            boss.Scale = 0.25f;
            boss.Active = false;
            shootingTimer = 3d;
            nonShootingTimer = 1d;
            hitPoints = 20;
            boss.Velocity = new Vector2(2, 0);
            boss.Position = new Vector2(550, 65);

            num5Spawns = EnemyType5.Count() % 20;
            swarmCount = (int)(EnemyType5.Count() / 20);
        ...
        public override void LoadContent()
        {
            ...
            music = content.Load<SoundEffect>("Audio/BSAudio/outer5");
        ...
        private void bossUpdate(GameTime gameTime)
        {
            for (int i = 0; i < bullets.Length; i++)
            {
                if (boss.Active && bullets[i].Active && bullets[i].BoundingRect.Intersects(boss.BoundingRect))
                {
                    hitPoints -= 1;
                    bullets[i].Active = false;
                }
            ...
            if (hitPoints <= 0 && boss.Active)
            {
                defeated = true;
                CreateExplosion(boss);
                boss.Position = new Vector2(0, -50f);
                boss.Active = false;
            }//end if
            ...
            if (shooting && boss.Active)
            {

                boss.Velocity = lastVel;
                nonShootingTimer -= gameTime.ElapsedGameTime.TotalSeconds;
                if (nonShootingTimer <= 0d)
                {
                ...
                bulletInterval -= gameTime.ElapsedGameTime.TotalSeconds;
                if (bulletInterval <= 0d)
                {
                    bool found = false;
                    for (int i = 0; i < enemyBullets.Length; i++)
                    {
                        if (!found && !enemyBullets[i].Active)
                        {
                            enemyBullets[i].Active = true;
                            enemyBullets[i].Velocity = new Vector2(0, 10f);
                            enemyBullets[i].Position = new Vector2(boss.Position.X + (boss.ActualWidth / 4), boss.Position.Y + boss.ActualHeight);
                            found = true;
                            gunShot.Play();
                        }//end if
                    }//end for
                    ...
                    for (int i = 0; i < enemyBullets.Length; i++)
                    {
                        if (!found && !enemyBullets[i].Active)
                        {
                            enemyBullets[i].Active = true;
                            enemyBullets[i].Velocity = new Vector2(0, 10f);
                            enemyBullets[i].Position = new Vector2(boss.Position.X + ((boss.ActualWidth / 4) * 3), boss.Position.Y + boss.ActualHeight);
                            found = true;
                        }//end if
                    }//end for
                    bulletInterval = 0.1d;
                ...
                if (boss.Position.X >= ScreenManager.GraphicsDevice.Viewport.Width - boss.ActualWidth - 30)
                {
                    boss.Velocity = new Vector2(-2f, 0);
                    lastVel = boss.Velocity;
                }

                else if (boss.Position.X <= 35)
                {
                    boss.Velocity = new Vector2(2f, 0);
                    lastVel = boss.Velocity;
                }
            ...
            if (player.BoundingRect.Intersects(boss.BoundingRect))
            {
                CreateExplosion(player);
                lives -= 1;
                player.Position = new Vector2(ScreenManager.Viewport.Width / 2,
            ...
            if (missle.Active & missle.BoundingRect.Intersects(boss.BoundingRect))
            {
                CreateExplosion(missle);
                hitPoints -= 3;
                missle.Active = false;
            }//end if
        }//end bossUpdate
                ...
                if (!active)
                {
                    ...
                    if (defeated)
                    {
                        ...
                        if (end < 0)
                        {
                            ...
                            ScreenManager.RemoveAd();
                            base.Remove();
                            LoadingScreen.Load(ScreenManager, true, new BSLevel6(score, lives, doubleBlaster, numMissles, blasterBonus, missleBonus, toBlasterBonus, toMissleBonus));
                        }//end if
                    }//end if
        ...
        {
            PhoneApplicationService.Current.State["boss"] = boss;
            PhoneApplicationService.Current.State["defeated"] = defeated;
            PhoneApplicationService.Current.State["shootingTimer"] = shootingTimer;
            PhoneApplicationService.Current.State["nonShootingTimer"] = nonShootingTimer;
            PhoneApplicationService.Current.State["bulletInterval"] = bulletInterval;
            PhoneApplicationService.Current.State["shooting"] = shooting;
            PhoneApplicationService.Current.State["notShooting"] = notShooting;
            PhoneApplicationService.Current.State["lastVel"] = lastVel;
            PhoneApplicationService.Current.State["hitPoints"] = hitPoints;
            ...
            PhoneApplicationService.Current.State["num5Spawns"] = num5Spawns;  //needed to ensure the swarm is accounted for
        ...
        {
            GameObject tempBoss = (GameObject)PhoneApplicationService.Current.State["boss"];
            boss.Position = tempBoss.Position;
            boss.Velocity = tempBoss.Velocity;
            boss.Active = tempBoss.Active;
            defeated = (bool)PhoneApplicationService.Current.State["defeated"];
            shootingTimer = (double)PhoneApplicationService.Current.State["shootingTimer"];
            nonShootingTimer = (double)PhoneApplicationService.Current.State["nonShootingTimer"];
            bulletInterval = (double)PhoneApplicationService.Current.State["bulletInterval"];
            shooting = (bool)PhoneApplicationService.Current.State["shooting"];
            notShooting = (bool)PhoneApplicationService.Current.State["notShooting"];
            lastVel = (Vector2)PhoneApplicationService.Current.State["lastVel"];
            hitPoints = (int)PhoneApplicationService.Current.State["hitPoints"];
            ...
            num5Spawns = (int)PhoneApplicationService.Current.State["num5Spawns"];  //needed to ensure the swarm is accounted for
