...
namespace PAYG
{
    ...
    class H1N1Level_11 : GameScreen
    {
        ...
        // Game Objects
        GameObject bars, background, paddle, paddleColision, blocks1, extraLifes;

        GameObject[] balls = new GameObject[1];

        GameObject[] blocks2 = new GameObject[2];
        GameObject[] blocks3 = new GameObject[2];

        GameObject[] pigRow1 = new GameObject[17];
        GameObject[] pigRow2 = new GameObject[15];
        GameObject[] pigRow3 = new GameObject[13];
        GameObject[] pigRow4 = new GameObject[9];
        ...
        public H1N1Level_11(bool active)
        {
            TransitionOnTime = TimeSpan.FromSeconds(2);
            TransitionOffTime = TimeSpan.FromSeconds(1);
            didRestart = active;
        ...
        public override void Initialize()
        {
            ...
            for (int a = 0; a < pigRow1.Length; a++)
            {
                ...
                pigRow1[a] = row1Pig;
                row1Pig.Position = new Vector2(41 + row1space, 100);
            ...
            for (int b = 0; b < pigRow2.Length; b++)
            {
                ...
                pigRow2[b] = row2Pig;
                row2Pig.Position = new Vector2(86 + row2Space, 145);
            ...
            for (int c = 0; c < pigRow3.Length; c++)
            {
                ...
                pigRow3[c] = row3Pig;
                row3Pig.Position = new Vector2(131 + row3Space, 190);
            ...
            for (int d = 0; d < pigRow4.Length; d++)
            {
                ...
                pigRow4[d] = row4Pig;
                row4Pig.Position = new Vector2(221 + row4Space, 235);
            ...
            for (int e = 0; e < pigRow5.Length; e++)
            {
                ...
                pigRow5[e] = row5Pig;
                row5Pig.Position = new Vector2(320 + row5Space, 280);
            ...
            blocks1 = new GameObject(SpriteSheet, new Rectangle(14, 26, 160, 32), 1, true, 0);
            blocks1.Position = new Vector2(380, 355);
            blocks1.Origin = new Vector2((blocks1.ActualWidth / 2) / blocks1.Scale, (blocks1.ActualHeight / 2) / blocks1.Scale);
            blocks1.Scale = .75f;
    

            for (int x = 0; x < blocks2.Length; x++)
            {
                GameObject block2 = new GameObject(SpriteSheet, new Rectangle(14, 26, 160, 32), 1, true, 0);
                blocks2[x] = block2;
                block2.Position = new Vector2(235 + block2RowSpace, 300);
            ...
            for (int x = 0; x < blocks3.Length; x++)
            {
                ...
                blocks3[x] = block3;
                block3.Position = new Vector2(130 + block3RowSpace, 235);
                ...
                extraLifes = new GameObject(SpriteSheet, new Rectangle(192, 11, 256, 256), 2, true, 0);
                extraLifes.AnimationSpeed = TimeSpan.FromMilliseconds(600);
                extraLifes.Position = new Vector2(380, 320);
                extraLifes.Origin = new Vector2((extraLifes.ActualWidth / 2) / extraLifes.Scale, (extraLifes.ActualHeight / 2) / extraLifes.Scale);
                extraLifes.Scale = 0.15f;
        ...
        public override void Update(GameTime gameTime, bool covered)
        {
         ...
         Accelerometer.GetState().Acceleration.Y, Accelerometer.GetState().Acceleration.Z);
            ...
            if (hud.Score >= highScore)
            {
                ...
                ScreenManager.RemoveAd();
                base.Remove();
                LoadingScreen.Load(ScreenManager, true, new H1N1Level_12());    
            }
            ...
            extraLifes.Update(gameTime);
            blocks1.Update(gameTime);
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            blocks1.Draw(gameTime, spriteBatch);
            ...
            extraLifes.Draw(gameTime, spriteBatch);          
        ...
        private void CheckBlock1Collision()
        {
            ...
            foreach (var ball in balls)
            {
                ...
                if (ball.BoundingRect.Intersects(blocks1.BoundingRect))
                {
                    effectWall.Play();

                    Vector2[] blockCorners = new Vector2[4];
                    blockCorners[0] = new Vector2(blocks1.Position.X - blocks1.Origin.X * blocks1.Scale, blocks1.Position.Y - blocks1.Origin.Y * blocks1.Scale);
                    blockCorners[1] = new Vector2(blocks1.Position.X + blocks1.Origin.X * blocks1.Scale, blocks1.Position.Y - blocks1.Origin.Y * blocks1.Scale);
                    blockCorners[2] = new Vector2(blocks1.Position.X + blocks1.Origin.X * blocks1.Scale, blocks1.Position.Y + blocks1.Origin.Y * blocks1.Scale);
                    blockCorners[3] = new Vector2(blocks1.Position.X - blocks1.Origin.X * blocks1.Scale, blocks1.Position.Y + blocks1.Origin.Y * blocks1.Scale);

                    BlockCornerMarkers = new float[4];

                    for (int i = 1; i < BlockCornerMarkers.Length; i++)
                    {
                        Vector2 direction = Vector2.Normalize(blockCorners[i] - blocks1.Position);
                        BlockCornerMarkers[i] = (float)Math.Atan2(direction.Y, direction.X) + MathHelper.Pi;
                    }

                    Vector2 direction2 = Vector2.Normalize(ball.Position - blocks1.Position);
            ...
            foreach (var ball in balls)
            {
                foreach (var block2 in blocks2)
                {
                    if (ball.BoundingRect.Intersects(block2.BoundingRect))
                    {
                        ...
                        Vector2[] blockCorners = new Vector2[4];
                        blockCorners[0] = new Vector2(block2.Position.X - block2.Origin.X * block2.Scale, block2.Position.Y - block2.Origin.Y * block2.Scale);
                        blockCorners[1] = new Vector2(block2.Position.X + block2.Origin.X * block2.Scale, block2.Position.Y - block2.Origin.Y * block2.Scale);
                        blockCorners[2] = new Vector2(block2.Position.X + block2.Origin.X * block2.Scale, block2.Position.Y + block2.Origin.Y * block2.Scale);
                        blockCorners[3] = new Vector2(block2.Position.X - block2.Origin.X * block2.Scale, block2.Position.Y + block2.Origin.Y * block2.Scale);

                        BlockCornerMarkers = new float[4];

                        for (int i = 1; i < BlockCornerMarkers.Length; i++)
                        {
                            Vector2 direction = Vector2.Normalize(blockCorners[i] - block2.Position);
                            BlockCornerMarkers[i] = (float)Math.Atan2(direction.Y, direction.X) + MathHelper.Pi;
                        }
        ...
        private void extraLifeCollision()
        {
            foreach (var ball in balls)
            {
                 if (ball.BoundingRect.Intersects(extraLifes.BoundingRect))
                    {
                        extraLifes.Active = false;
                        extraLifes.Position = new Vector2(1300, 750);
        ...
        protected void SaveState(object sender, DeactivatedEventArgs args)
        {
            ...
            PhoneApplicationService.Current.State["extraLifes"] = extraLifes;

            PhoneApplicationService.Current.State["blocks1"] = blocks1;
            PhoneApplicationService.Current.State["blocks2"] = blocks2;
            PhoneApplicationService.Current.State["blocks3"] = blocks3;
        ...
        public void LoadLevel()
        {
            ...
            GameObject blocks1Load = (GameObject)PhoneApplicationService.Current.State["blocks1"];
            blocks1.Position = blocks1Load.Position;
            
            GameObject[] blocks2Load = (GameObject[])PhoneApplicationService.Current.State["blocks2"];
            for (int i = 0; i < blocks2.Length; i++)
            {
                blocks2[i].Position = blocks2Load[i].Position;
            }
            GameObject[] blocks3Load = (GameObject[])PhoneApplicationService.Current.State["blocks3"];
            for (int i = 0; i < blocks3.Length; i++)
            {
                blocks3[i].Position = blocks3Load[i].Position;
            }

            GameObject extraLifesLoad = (GameObject)PhoneApplicationService.Current.State["extraLifes"];

                extraLifes.Position = extraLifesLoad.Position;
                extraLifes.AnimationSpeed = TimeSpan.FromMilliseconds(600);
